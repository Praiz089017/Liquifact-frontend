...
jest.mock(
  ...
  () =>
    function StatusPillMock({ status }: { status: string }) {
      return <span role="status" data-status={status}>{status}</span>;
    }
);
...
beforeEach(() => {
  ...
  mockUseWallet.mockReturnValue({ state: "disconnected", connect: jest.fn() } as ReturnType<typeof useWallet>);

  // Replace location so origin is predictable
...
afterEach(() => {
  (window as unknown as { location: unknown }).location = ORIGINAL_LOCATION;
});
...
describe("InvoiceDetailPage (Server Component shell)", () => {
  ...
  describe("params contract", () => {
    it("accepts a plain params object (current Next.js form)", async () => {
      const jsx = await InvoiceDetailPage({ params: { id: "inv-001" } as unknown as Promise<{ id: string }> });
  ...
  describe("JSON-LD sanitization", () => {
    ...
    it("returns null JSON-LD when invoice is falsy (defensive)", async () => {
      ...
      const { buildInvoiceJsonLdExported } = await import("./page") as unknown as { buildInvoiceJsonLdExported?: (i: null) => null };
  ...
  describe("copy link button", () => {
    ...
    it("copies the invoice URL to clipboard and shows a success toast", async () => {
      ...
      expect(writeText).toHaveBeenCalledWith(
        expect.stringContaining("/invest/inv-001")
      );
...
describe("copyToClipboardFallback", () => {
  ...
  it("always removes the textarea even when execCommand throws", () => {
    const removeChild = jest.spyOn(document.body, "removeChild");
    document.execCommand = jest.fn(() => { throw new Error("Not supported"); });
